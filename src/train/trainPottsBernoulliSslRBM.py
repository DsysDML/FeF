...
class RBMdataset(Dataset):
    def __init__(self, file_path, dataset='train', partial_labels=True, lab_frac=0.5, device=torch.device('cpu')):
        f = File(file_path, 'r')
        self.file_path = file_path
        self.data = torch.tensor(f[dataset][()]).type(torch.int64).to(device)
...
if args.train_mode == 'new':
    ...
    train_dataset = RBMdataset(args.data, dataset='train', partial_labels=args.partial_labels, lab_frac=args.lab_frac, device=args.device_dataset)
